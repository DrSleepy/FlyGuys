...
class Validation {
  ...
  // function is public because it is the access point for class
  ...
  private function subRuleSplitter() {
  ...
  // runs each sub rule name against its own set rules
  private function subRuleCriteria() {
    switch ($this->subRuleName) {
      ...
      case 'postcode':
        ...
        break;
      case 'uniqueEmail':
        $email = $this->ruleValue;
        $emailExists = $this->db->getUserByEmail($email);

        if ($emailExists[0] > 0) {
          $this->errors[$this->ruleName][] = "email already in use";
        }
      break;
      case 'telephone':
