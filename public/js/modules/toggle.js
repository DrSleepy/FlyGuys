export default (id, element) => {
  ...
  if (toggler === event.target || toggler.contains(event.target)) {
    ...
    toToggle.style.display === 'inline' ? (toToggle.style.display = 'none') : (toToggle.style.display = 'inline');
  } else {
    // if click is not on id or child nodes, hide element node
    if (!toggler === event.target || !toToggle === event.target || !toToggle.contains(event.target)) {
